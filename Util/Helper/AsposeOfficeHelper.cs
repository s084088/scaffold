...
namespace Util
{
    ...
    public class AsposeOfficeHelper
    {
        ...
        public static MemoryStream DataTableToExcel(DataTable dt)
        {
            ...
            //生成数据行
            for (int i = 0; i < Rownum; i++) for (int k = 0; k < Colnum; k++) cells[1 + i, k].PutValue(dt.Rows[i][k].ToString());
        ...
        {
            return DataTableToExcel(dt).ToArray();
        }
        ...
        /// <returns></returns>
        public static DataTable ReadExcel(string fileName, int startRowIndex)
        {
            Workbook book = new Workbook(fileName);
            Worksheet sheet = book.Worksheets[0];
            Cells cells = sheet.Cells;

            return cells.ExportDataTable(startRowIndex, 0, cells.MaxDataRow - 1, cells.MaxDataColumn + 1);
            //return cells.ExportDataTable(startRowIndex, 0, cells.MaxDataRow, cells.MaxDataColumn + 1, false);
        }
